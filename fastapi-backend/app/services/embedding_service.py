...
class FallbackEmbeddings(Embeddings):
    ...
    def _generate_vector(self, text: str) -> List[float]:
        # Generate seed from SHA-256 hash of text
        hash_bytes = hashlib.sha256(text.encode("utf-8")).digest()
        
        vector = []
        for i in range(TARGET_VECTOR_DIM):
            # Deterministic pseudo-random float generator from hash bytes & index
            byte_idx = (i * 2) % len(hash_bytes)
            val = (hash_bytes[byte_idx] + hash_bytes[(byte_idx + 1) % len(hash_bytes)] * 256) / 65535.0
            # Scale to [-1.0, 1.0] with sinusoidal modulation
...
class EmbeddingService:
    ...
    def _initialize_model(self) -> Embeddings:
        ...
        logger.info("Using FallbackEmbeddings (deterministic 1536-dim vector generator).")
        return FallbackEmbeddings()

    def _normalize_dimension(self, vector: List[float], target_dim: int = TARGET_VECTOR_DIM) -> List[float]:
        """
        Ensures output vector matches PostgreSQL vector(1536) column definition.
        """
        curr_dim = len(vector)
